/**
* Original made by Pisan
* Edited by Daniel Yan
* Pre-conditions on Graph:
...
public:
	...
	* */
	bool Connect(const std::string &fromLabel,
		const std::string &toLabel, int edgeWeight);
...
private:
	...
	void bfsHelper(Vertex *v, void visit(const std::string &));
	...
	*/
	Vertex *createVertex(const std::string &vertexLabel);
	...
	* @return nothing
	*/
	void sortEdges(std::string &fromLabel);
